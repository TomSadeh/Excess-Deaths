...
df['2.50%'] = df.loc[:,'2010':'2020'].quantile(0.025, axis = 'columns')
df['97.50%'] = df.loc[:,'2010':'2020'].quantile(0.975, axis = 'columns')
...
df['Average 2010-2012'] = df.loc[:,'2010':'2012'].mean(axis = 'columns')
...
df['rolling2010-2012'] = df['Average 2010-2012'].rolling(x, min_periods = w).mean()
...
#plt.plot(df['Date'], df['rolling2010-2012'], color = 'tab:olive', alpha = 0.5, label = invert('ממוצע 0102-2102'))
...
plt.annotate(areaname,(90,15), xytext = (120,15), arrowprops = dict(arrowstyle = '->', alpha = 0.75))
...
plt.title(invert('שיעור תמותה יומי מתוקנן לגיל בישראל למיליון נפש בשנים 0202-0102'))
plt.ylabel(invert(' ימים') + str(x) + invert('שיעור תמותה, ממוצע נע של '))
plt.xlabel(invert('חודש'))
plt.legend()


plt.savefig(r'C:\Users\User\Documents\Projects\Daily Excess Deaths\fig_excess_deaths2.png', dpi =300)